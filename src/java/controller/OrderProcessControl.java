...
public class OrderProcessControl extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        if (cus != null) {
            count = ohd.getTimesOfUserId(cus.getCustomerId(), cus.getRoleId());

            if (count > 0) {
                address = ohd.getAddressOrder(cus.getCustomerId(), cus.getRoleId());
            } else {
            ...
            count = ohd.getTimesOfUserId(ad.getAdminId(), ad.getRoleId());

            if (count > 0) {
                address = ohd.getAddressOrder(ad.getAdminId(), ad.getRoleId());
            } else {
                address = cus.getCustomerAddress();
            }
            request.setAttribute("address", address);
